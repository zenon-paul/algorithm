...
void deletenochild(struct d_node* current, struct d_node* parent) {
	if (parent != NULL)
	{
...
void deletetwochild(struct d_node* current) {
	struct d_node* max;
	...
	if (max->left == NULL)
	{
		deletenochild(max, maxparent);
	}
	else
	{
		deleteonechild(max, max->left);
	}
}
int deletenode(int num) {
	...
	else
	{
		if ((current->left == NULL)&&(current->right == NULL)) {
			deletenochild(current,parent);
		}
		else if((current->left == NULL)&&(current->right != NULL)) {
			deleteonechild(current,current->right);
		}
		else if ((current->right == NULL) && (current->left != NULL)) {
			deleteonechild(current, current->left);
		}
		else {
