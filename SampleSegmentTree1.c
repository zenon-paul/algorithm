...
int* init_segtree(int* x, int size, int* n) {
	...
	*n = leavs;
	segtree = (int*)malloc(sizeof(int)*( 2 * leavs - 1));
...
int* init_lazy(int leavs) {
	int* x;
	x = (int*)malloc(sizeof(int)*(2 * leavs - 1));
	for (int i = 0; i < (2 * leavs - 1); i++)
	{
		x[i] = 0;
	}
...
void add(int* segtree, int* lazy, int num, int s, int g, int current, int l, int r) {
	...
	else
	{
		...
		add(segtree, lazy, num, s, g, 2 * current + 2, l + (r - l) / 2, r);
...
int get_sum(int* segtree, int* lazy, int s, int g, int current, int l, int r) {
	...
	else
	{
		...
		int y = get_sum(segtree, lazy, s, g, 2 * current + 2, l + (r - l) / 2, r);
		return x + y;
	}
}
...
int main() {
	...
	disp(body, leavs);
	disp(attached, leavs);
	add(body, attached, 2, 5, 12, 0, 0, leavs);
